...
impl Config {
    pub(crate) fn parse_from(mut args: impl Iterator<Item = String>) -> Result<Self, String> {
        if args.next().as_deref() != Some("vthread") {
            return Err(usage());
        }
        let scenario = match args.next().as_deref() {
            ...
            Some("channel-mpmc") => Scenario::ChannelMpmc {
                per_task: positive(&mut args, "messages-per-task")?,
...
fn common_usage() -> String {
    "usage: vthread-benchmarks vthread yield <yields-per-task> <workers> <tasks> <odd-samples>\n       vthread-benchmarks vthread spawn <workers> <tasks> <odd-samples>\n       vthread-benchmarks vthread park <parks-per-task> <workers> <even-tasks> <odd-samples>\n       vthread-benchmarks vthread mutex <locks-per-task> <workers> <tasks> <odd-samples>\n       vthread-benchmarks vthread mutex-uncontended <locks-per-task> <workers> 1 <odd-samples>\n       vthread-benchmarks vthread channel <messages-per-task> <workers> <even-tasks> <odd-samples>\n       vthread-benchmarks vthread channel-bounded-spsc <messages-per-task> <capacity> <workers> <even-tasks> <odd-samples>\n       vthread-benchmarks vthread tcp <round-trips-per-task> <workers> <tasks> <odd-samples>\n       vthread-benchmarks vthread wake-tail <wakes-per-task> <workers> <even-tasks> <odd-samples>\n       vthread scenarios also accept: --max-vthreads <capacity> and --pin-carriers (Linux only)".into()
}
