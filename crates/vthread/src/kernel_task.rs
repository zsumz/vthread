...
use super::Task;
use crate::{context, control::Shared};
use std::rc::Rc;
use vthread_stack::{FiberState, Suspension};

impl Task {
    #[inline]
    pub(super) fn dispatch(&mut self, shared: &Shared) -> Option<FiberState> {
        let state = {
        ...
        match &state {
            Some(FiberState::Suspended(Suspension::YieldNow)) => self
                .execution
                .progress
                .yield_now(self.execution.record.progress()),
            Some(FiberState::Suspended(Suspension::Park(_))) => self
                .execution
                .progress
                .park(self.execution.record.progress()),
            Some(FiberState::Complete) => self
                .execution
                .progress
                .unmount(self.execution.record.progress()),
            None => {}
        }
        state
    }
