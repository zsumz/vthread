        ...
        .scope(|scope| {
            scope
                .spawn("parent", || {
                    ...
                    local_scope(|local| {
                        let first = local.spawn("receive", || receiver.recv())?;
                        ...
                        let second = local.spawn("send", || sender.send(3))?;
                        while sender.waiting() == 0 {
                            yield_now()?;
                        }
                        assert_eq!(receiver.recv()?, 2);
                        let error = sender.send(4).unwrap_err();
                        assert!(matches!(error.error, Error::WaitQueueFull { limit: 1 }));
...
fn shutdown_removes_both_directions_and_reclaims_unsent_input() {
    ...
    runtime
        .scope(|scope| {
            ...
            let send_task = scope.spawn("send", move || sending.send(Count(tracked)))?;
            let receiving = other_receiver.clone();
            let recv_task = scope.spawn("recv", move || receiving.recv())?;
